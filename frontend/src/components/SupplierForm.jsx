...
import { X, Save, Package, MapPin, Phone, Mail } from 'lucide-react'
...
export default function SupplierForm({ isOpen, onClose, supplier, onSuccess }) {
  ...
  useEffect(() => {
    if (supplier) {
      setFormData({
        ...
        adresse_courriel: supplier.adresse_courriel || ''
      })
    } else {
      setFormData({
        nom: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (supplier) {
        await suppliersService.updateSupplier(supplier.id_fournisseur, submitData)
        onSuccess()
      } else {
